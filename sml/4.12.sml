(* 
 * (medium) Construct a complete binary tree
 *
...
fun countNodes Empty = 0
  | countNodes (Node (_, l, r)) = 1 + countNodes l + countNodes r

fun fullBinaryTree 1 = Node ("", Empty, Empty)
  | fullBinaryTree n = Node ("", fullBinaryTree (n - 1), fullBinaryTree (n - 1))

fun completeBinaryTree n =
  let
    fun aux i =
      if i > n
      then Empty
      else Node ("", aux (2 * i), aux (2 * i + 1))
  in
    aux 1
  end
